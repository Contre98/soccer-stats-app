...
function* getCombinations<T>(array: T[], k: number): Generator<T[]> { /* ... */
    // console.log(`getCombinations called with array size ${array.length}, k=${k}`); // DEBUG
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
            ...
            <div className="flex justify-between items-center mb-4"><h2 className="text-xl font-semibold text-gray-800 dark:text-white flex items-center"><Users className="w-6 h-6 mr-2 text-blue-600 dark:text-blue-400" /> Select Players</h2><div className="flex space-x-2"><button onClick={handleSelectAll} className="p-1 text-xs text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 disabled:opacity-50" title="Select All" disabled={availablePlayers.length === 0 || isLoading}><CheckSquare className="w-4 h-4"/></button><button onClick={handleDeselectAll} className="p-1 text-xs text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 disabled:opacity-50" title="Deselect All" disabled={selectedPlayerIds.size === 0 || isLoading}><Square className="w-4 h-4"/></button></div></div>
            {/* ... Player List ... */}
            <div className="max-h-[500px] overflow-y-auto space-y-3 pr-2">{availablePlayers.length === 0 && (<p className="text-sm text-gray-500 dark:text-gray-400">No players found...</p>)}{availablePlayers.map((player) => (<label key={player.id} className="flex items-center space-x-3 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer transition duration-150 ease-in-out"><input type="checkbox" checked={selectedPlayerIds.has(player.id)} onChange={() => handlePlayerSelectionChange(player.id)} className="h-5 w-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500 dark:bg-gray-600 dark:border-gray-500"/><span className="text-sm font-medium text-gray-900 dark:text-white">{player.name}</span><span className="ml-auto text-xs text-gray-500 dark:text-gray-400 flex items-center"><Star className="w-3 h-3 mr-1 text-yellow-400" /> {player.manual_rating}</span></label>))}</div>
            {/* ... Selection Counter ... */}
            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700"><p className={`text-sm font-medium ${totalSelected === requiredPlayers ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>Selected: {totalSelected} / {requiredPlayers}</p></div>
        </div>
        {/* Column 2: Options & Generated Teams */}
        <div className="md:col-span-2 space-y-6">
          {/* ... Team Size Selection ... */}
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700"><h2 className="text-xl font-semibold mb-4 text-gray-800 dark:text-white">Select Format</h2><div className="flex flex-wrap gap-3">{teamSizes.map((size) => (<button key={size} onClick={() => handleTeamSizeChange(size)} className={`px-4 py-2 rounded-md text-sm font-medium transition duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-800 focus:ring-blue-500 ${ selectedTeamSize === size ? 'bg-blue-600 text-white shadow' : 'bg-gray-200 text-gray-700 hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600' }`}>{size}v{size}</button>))}</div></div>
          {/* ... Generate Button ... */}
          <button onClick={handleGenerateTeams} disabled={isLoading || totalSelected !== requiredPlayers} className="w-full flex items-center justify-center px-6 py-3 bg-green-600 text-white rounded-lg shadow hover:bg-green-700 transition duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 disabled:opacity-50 disabled:cursor-not-allowed">{isLoading ? <Loader2 className="w-5 h-5 mr-2 animate-spin" /> : <Shuffle className="w-5 h-5 mr-2" />}{isLoading ? 'Generating...' : 'Generate Teams'}</button>
      ...
      <SaveGeneratedMatchModal isOpen={isSaveModalOpen} onClose={handleCloseSaveModal} onSave={handleSaveGeneratedMatch} teamA={optionToSave?.teamA ?? []} teamB={optionToSave?.teamB ?? []} />
    </div>
  );
}
