...
import { PlusCircle, Youtube, ExternalLink, Trash2, Edit, AlertCircle } from 'lucide-react'; // Added AlertCircle
...
// --- Import Shared Types ---
import type { PlayerInfo, MatchPlayerInfo, MatchWithPlayers } from '@/lib/types'; // Import shared types
...
// Type for data coming from EditMatchModal - use Pick from shared type
...
interface MatchesClientComponentProps {
  ...
  availablePlayers: PlayerInfo[]; // Use PlayerInfo for available players list
  currentPage: number;
  totalPages: number;
...
export default function MatchesClientComponent({
  ...
  // Update local state when initialMatches prop changes (due to pagination navigation)
  useEffect(() => {
  ...
  const handleSaveMatch = async (matchData: AddMatchDataType) => {
    ...
    const matchInsertData = {
      match_date: matchData.match_date, score_a: matchData.score_a,
      score_b: matchData.score_b, replay_url: matchData.replay_url, user_id: user.id,
    };
    try {
      await addMatchWithPlayers(supabase, matchInsertData, matchData.teamAPlayerIds, matchData.teamBPlayerIds);
      alert("Match saved successfully!");
      router.push('/matches?page=1'); // Go to page 1 after adding
    } catch (error) {
  ...
  const handleUpdateMatch = async (id: number, updatedData: UpdateMatchDataType) => {
     try {
       const { data: updatedMatch, error: updateError } = await supabase
         ...
         .update(updatedData)
         .match({ id: id })
         // Re-fetch with the same nested structure
         .select(`*, match_players ( team, players ( id, name ) )`)
         .single();
       if (updateError || !updatedMatch) { throw updateError || new Error("No data returned after update."); }
       // Update local state with the correctly typed data
  ...
  // --- DELETE MATCH Handler ---
   const handleDeleteMatch = async (matchId: number) => {
      ...
      const { error: deleteError } = await supabase.from('matches').delete().match({ id: matchId });
      if (deleteError) {
          console.error('Error deleting match:', deleteError);
          alert(`Error deleting match: ${deleteError.message}`);
  ...
  // --- Helper to get player names from MatchPlayerInfo ---
  const getPlayerNames = (matchPlayers: MatchPlayerInfo[], team: 'A' | 'B'): string => {
      return matchPlayers
          // Filter for the correct team
          .filter(mp => mp.team === team)
          // Access the nested players array, get the first player's name (if exists)
          // --- UPDATED Fallback: Access ID from nested players array ---
          .map(mp => mp.players?.[0]?.name ?? `ID:${mp.players?.[0]?.id ?? '?'}`)
          // --- END UPDATE ---
          .join(', ') || `Team ${team} Players (N/A)`; // Fallback display
  };
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {error && (
        <div className="mb-4 p-4 text-sm text-red-700 bg-red-100 rounded-lg dark:bg-red-900/30 dark:text-red-400 border border-red-300 dark:border-red-600" role="alert">
           <span className="font-medium flex items-center"> <AlertCircle className="w-4 h-4 mr-2"/> Error loading matches:</span> {error}
        </div>
      )}
          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {!error && matches.length === 0 ? (
              ...
              matches.map((match) => {
                 ...
                 const teamADisplay = getPlayerNames(match.match_players, 'A');
                 const teamBDisplay = getPlayerNames(match.match_players, 'B');
                 return (
            ...
            {error && (
                <tr><td colSpan={5} className="px-6 py-10 text-center text-sm text-red-600 dark:text-red-400">Could not load matches. Please try again later.</td></tr>
             )}
          </tbody>
        </table>
      </div>
      ...
      {!error && totalPages > 1 && (
        <div className="flex justify-center items-center space-x-4 mt-6">
          <Link href={`/matches?page=${currentPage - 1}`} passHref legacyBehavior>
            <a className={`px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md shadow ${currentPage <= 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-700'}`} aria-disabled={currentPage <= 1} onClick={(e) => { if (currentPage <= 1) e.preventDefault(); }}>Previous</a>
          </Link>
          <span className="text-sm text-gray-700 dark:text-gray-300">Page {currentPage} of {totalPages}</span>
          <Link href={`/matches?page=${currentPage + 1}`} passHref legacyBehavior>
            <a className={`px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md shadow ${currentPage >= totalPages ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-700'}`} aria-disabled={currentPage >= totalPages} onClick={(e) => { if (currentPage >= totalPages) e.preventDefault(); }}>Next</a>
          </Link>
        </div>
      )}
